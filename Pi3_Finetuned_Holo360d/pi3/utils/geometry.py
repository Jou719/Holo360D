...
def get_pixel(H, W):
    # Generate 2D pixel coordinates in camera space.
...
def depthmap_to_absolute_camera_coordinates(depthmap, camera_intrinsics, camera_pose, z_far=0, **kw):
    ...
    if camera_pose is not None:
        ...
        # Express coordinates in the world frame.
...
def depthmap_to_camera_coordinates(depthmap, camera_intrinsics, pseudo_focal=None):
    ...
    # Compute the 3D ray associated with each pixel.
    # Assume there are no skew terms.
    ...
    # Depths above 80m are treated as invalid.
...
def warp_kpts(kpts0, depth0, depth1, T_0to1, K0, K1, smooth_mask = False, return_relative_depth_error = False, depth_interpolation_mode = "bilinear", relative_depth_error_threshold = 0.05):
    ...
    # Adapted from the LoFTR geometry utilities.
    Args:
    ...
    if depth_interpolation_mode == "combined":
        # Fill holes by combining bilinear and nearest-neighbor interpolation.
        if smooth_mask:
    ...
    # Sample depth and derive the valid mask from depth > 0.
